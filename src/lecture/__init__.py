...
def _caller_line() -> int | None:
    ...
    try:
        # [0]=_caller_line, [1]=SDK fn, [2]=author code
        outer = pyinspect.getouterframes(frame, 0)
        if len(outer) >= 3:
            return outer[2].lineno
    except Exception:
        return None
    finally:
        del frame
...
def _emit(kind: str, payload: dict[str, Any]) -> Event:
    ctx = require_current()
    return ctx.emit(kind, payload, line=_caller_line())
...
def inspect_value(name: str, value: Any) -> Event:
    ...
    ctx = require_current()
    return ctx.inspect(name, value, line=_caller_line())
...
def system_text(command: list[str] | str, *, max_bytes: int = 64_000) -> Event:
    ...
    try:
        ...
        return ctx.emit("error", {"message": f"system_text failed: {e}"}, line=_caller_line())
    ...
    return ctx.emit(
        "terminal", {"argv": argv, "output": clean, "exit_code": 0}, line=_caller_line()
    )
