...
function classSvg(spec) {
  ...
  let content = `<defs><marker id="uml-arrow" markerWidth="10" markerHeight="10" refX="8" refY="4" orient="auto"><path d="M0 0L8 4L0 8Z" fill="CanvasText"/></marker></defs>`;
  relations.forEach((relation) => {
    ...
    content += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="CanvasText" stroke-width="2"${dash} marker-end="url(#uml-arrow)"/>`;
    if (relation?.label) content += `<text x="${(x1 + x2) / 2}" y="${(y1 + y2) / 2 - 8}" text-anchor="middle" font-size="14" fill="CanvasText">${umlEscape(label(relation.label))}</text>`;
  });
  classes.forEach((entry) => {
    const box = positions.get(entry.id);
    const attrY = box.y + 54;
    const methodY = attrY + Math.max(entry.attributes.length, 1) * 24;
    content += `<g class="uml-class"><rect x="${box.x}" y="${box.y}" width="${box.w}" height="${box.h}" fill="Canvas" stroke="CanvasText" stroke-width="2"/><rect x="${box.x}" y="${box.y}" width="${box.w}" height="54" fill="color-mix(in srgb, CanvasText 12%, Canvas)" stroke="CanvasText" stroke-width="2"/><text x="${box.x + box.w / 2}" y="${box.y + 33}" text-anchor="middle" font-size="19" font-weight="700" fill="CanvasText">${umlEscape(entry.name)}</text><line x1="${box.x}" y1="${methodY}" x2="${box.x + box.w}" y2="${methodY}" stroke="color-mix(in srgb, CanvasText 55%, Canvas)"/>`;
    (entry.attributes.length ? entry.attributes : ["—"]).forEach((value, index) => {
      content += `<text x="${box.x + 14}" y="${attrY + 18 + index * 24}" font-size="14" fill="CanvasText">${umlEscape(value)}</text>`;
    });
    (entry.methods.length ? entry.methods : ["—"]).forEach((value, index) => {
      content += `<text x="${box.x + 14}" y="${methodY + 18 + index * 24}" font-size="14" fill="CanvasText">${umlEscape(value)}</text>`;
    });
    content += "</g>";
  });
...
function sequenceSvg(spec) {
  ...
  participants.forEach((participant, index) => {
    ...
    content += `<rect x="${x - 64}" y="24" width="128" height="42" rx="3" fill="color-mix(in srgb, CanvasText 12%, Canvas)" stroke="CanvasText" stroke-width="2"/><text x="${x}" y="51" text-anchor="middle" font-size="16" font-weight="700" fill="CanvasText">${umlEscape(participant.name)}</text><line x1="${x}" y1="66" x2="${x}" y2="${height - 24}" stroke="color-mix(in srgb, CanvasText 55%, Canvas)" stroke-dasharray="7 6" stroke-width="2"/>`;
  });
  content += `<defs><marker id="uml-seq-arrow" markerWidth="10" markerHeight="10" refX="8" refY="4" orient="auto"><path d="M0 0L8 4L0 8Z" fill="CanvasText"/></marker></defs>`;
  messages.forEach((message, index) => {
    ...
    content += `<line x1="${x1}" y1="${y}" x2="${x2}" y2="${y}" stroke="CanvasText" stroke-width="2"${dashed} marker-end="url(#uml-seq-arrow)"/><text x="${(x1 + x2) / 2}" y="${y - 9}" text-anchor="middle" font-size="14" fill="CanvasText">${umlEscape(label(message?.label, "message"))}</text>`;
  });
